...
function create_fragment(ctx) {
	...
	return {
		...
		l(nodes) {
			...
			t1 = claim_text(button_nodes, "Change Media");
		...
		m(target, anchor) {
			...
			if (!mounted) {
				dispose = listen(button, "click", prevent_default(/*swapMedia*/ ctx[1]));
				mounted = true;
			}
		},
		p(ctx, [dirty]) {
...
function instance($$self, $$props, $$invalidate) {
	...
	const swapMedia = () => {
		$$invalidate(6, originalMedia = field);
		$$invalidate(3, changingMedia = field);
		$$invalidate(4, showMediaModal = true);
	};
	...
	$$self.$$set = $$props => {
		...
		if ("showMediaModal" in $$props) $$invalidate(4, showMediaModal = $$props.showMediaModal);
		if ("changingMedia" in $$props) $$invalidate(3, changingMedia = $$props.changingMedia);
		if ("localMediaList" in $$props) $$invalidate(5, localMediaList = $$props.localMediaList);
	};

	$$self.$$.update = () => {
		if ($$self.$$.dirty & /*changingMedia, field, originalMedia*/ 73) {
			$: if (changingMedia) {
				if (field === originalMedia) {
					$$invalidate(0, field = changingMedia);
				}
			}
		}
	};
